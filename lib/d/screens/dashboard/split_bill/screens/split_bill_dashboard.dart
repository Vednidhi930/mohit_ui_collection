...
import '../fragments/contact_split_bill_fragment.dart';
import '../fragments/home_split_bill_fragment.dart';
import '../fragments/payment_split_bill_fragment.dart';
import '../fragments/search_split_bill_fragment.dart';
...
class _SplitBillDashboardState extends State<SplitBillDashboard> {
  ...
  Widget _bottomTabBar() {
    return BottomNavigationBar(
      ...
      onTap: _onTap,
      backgroundColor:
          appStore.isDarkModeOn ? bottomNavBackgroundColor : Colors.white,
  ...
  Widget customBottomNav(BuildContext context, bool isActiveIcon, String icon) {
    if (isActiveIcon) {
      return commonCachedNetworkImage(icon,
          height: 20,
          width: 20,
          color: appStore.isDarkModeOn
              ? primarySplitBillColor
              : primarySplitBillColor);
    } else {
      return commonCachedNetworkImage(icon,
          height: 16,
          width: 16,
          color: appStore.isDarkModeOn ? bottomNavBarColor : Colors.black);
    }
  }
